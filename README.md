...
6. **상담 데스크** — 대상·경로·테마·목적으로 추천 키트 설계, 학부모 성장 리포트·FutureLab 6주·캠프 5일·교사 체크리스트 복사
...
앱 안에서 이미 가능한 것: **상담 데스크**에서 진단 → 추천 키트 → 학부모 리포트 복사, FutureLab/캠프 템플릿, 교사 체크리스트.

아직 앱 밖 연결이 필요한 것:

1. 홈페이지/포털에 이 앱 링크 연결  
2. 실제 신청·결제·학생 기록 시스템과 연동  
3. 상담 결과를 학부모 계정/리포트 DB에 저장  
4. 교사 관찰 기록을 수업 루브릭과 실시간 연동  
